...
Write the skill body as instructions to the agent, not to the user: steps the agent leads the user through, ending when the plugin is usable. Limit its `description` to the setup of this plugin, so it never fires on unrelated tasks. Body rules: `writing-for-the-agent.md`.
